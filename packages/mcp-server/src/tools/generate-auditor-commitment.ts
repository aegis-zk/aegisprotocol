...
`);
          ...
          // Read the return value from the execution output
          // nargo execute prints the public outputs
          const output = run('nargo execute 2>&1');

          // Parse the commitment from Verifier.toml (generated by nargo execute)
          const verifierToml = readFileSync(join(tempDir, 'Verifier.toml'), 'utf-8');
          const match = verifierToml.match(/return\s*=\s*"([^"]+)"/);

          if (!match) {
            throw new Error('Failed to extract commitment from nargo output. Verifier.toml: ' + verifierToml);
          }

          const commitment = match[1].startsWith('0x') ? match[1] : '0x' + match[1];
